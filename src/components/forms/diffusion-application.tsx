...
import {
  ...
  normalizeImage,
  rgbToGrayscale
} from '../../core/images/image-helpers'
...
const DiffusionApplication = () => {
  ...
  const handleAlgorithmChange = (algorithmName: string) => {
    ...
    const defaultParams = algorithm
      ? algorithm.parameters.reduce(
          (acc, param) => {
            acc[param.label] =
              typeof param.value === 'number' ? param.value : parseFloat(param.value)
            return acc
          },
    ...
    setUploadedImages((prevImages) => {
      ...
      if (remainingImages.length > 0) {
        ...
        const newSelectedIndex =
          indexToDelete < remainingImages.length ? indexToDelete : remainingImages.length - 1
  ...
  const applyImageProcessing = useCallback(
    (processingFunc: (imageData: ImageData) => ImageData) => {
  ...
  const handleResetCanvas = () => {
    if (selectedImage) {
      ...
      // Reset paintedAreas
      setPaintedAreas(
        Array(canvasRef.current?.height || 0)
          .fill(null)
          .map(() => Array(canvasRef.current?.width || 0).fill(false))
      )
    }
  }
    ...
    <div className="flex justify-center w-full">
          ...
          <div className="bg-gray-100 p-6 rounded-lg shadow-md">
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-4">
                ...
                <div className="flex-1">
                  <label htmlFor="gamma" className="block text-sm font-medium text-gray-700 mb-1">
                    Gamma
                  </label>
                  <input
                    type="range"
        ...
        <div className="bg-gradient-to-r from-blue-100 to-purple-100 p-6 rounded-lg shadow-lg">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-4">
              <label htmlFor="square-size" className="text-sm font-medium text-gray-700">
                Square Size:
              </label>
              <input
                type="range"
      ...
      >
        ...
        <div className="absolute top-5 right-5 flex gap-2.5">
          ...
          <button className="zoom-control-button" onClick={handleZoomReset}>
            Reset
          </button>
        </div>
      </div>
