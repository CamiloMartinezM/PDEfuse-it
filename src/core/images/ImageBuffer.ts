...
export class ImageBuffer {
  ...
  private initializePixelMatrix(width: number, height: number, type: ImageType): number[][][] {
    ...
    else throw new Error('Image type is not supported')
  ...
  static fromMatrix(matrix: number[][][]): ImageBuffer {
    ...
    if (matrix[0][0].length === 1) type = ImageType.GRAYSCALE
    else if (matrix[0][0].length === 3) type = ImageType.RGB
    else type = ImageType.MULTICHANNEL
  ...
  static pixelMagnitude(pixel: number[], sqrt: boolean = false): number {
    let sumOfSquares = 0
    for (let i = 0; i < pixel.length; i++) sumOfSquares += pixel[i] ** 2
  ...
  trimPadding(paddingSize: number): void {
    ...
    if (newWidth <= 0 || newHeight <= 0)
      throw new Error(
        'Padding size is too large, resulting in non-positive dimensions for the trimmed image.'
      )
  ...
  averageGreyLevel(): number {
    ...
    this.matrix.forEach((row) => {
      row.forEach((pixel) => {
        ...
        } else if (this.type === ImageType.GRAYSCALE) greyValue = pixel[0]
        else throw new Error('Image type is not supported')
  ...
  findMaxPixel(): number[] {
    ...
    this.matrix.forEach((row) => {
      row.forEach((pixel) => {
        ...
        if (currentMaxMagnitude !== maxMagnitude) max = pixel
      })
    })
    return max
  }
  ...
  findMinPixel(): number[] {
    ...
    this.matrix.forEach((row) => {
      row.forEach((pixel) => {
        ...
        // Update the max pixel if the current pixel has a greater magnitude
        if (currentMinMagnitude !== minMagnitude) min = pixel
      })
    })
    return min
  }
