 ...
 */
export const DiffusionAlgorithmSettings: React.FC<DiffusionAlgorithmSettingsProps> = ({
  onApply,
  onAlgorithmChange
}) => {
  ...
  const handleAlgorithmChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (newAlgorithm) {
      ...
      onAlgorithmChange(
        newAlgorithm.name,
        newAlgorithm.parameters.reduce(
          (acc, param) => {
  ...
  const handleApply = () => {
    ...
    const params: { [key: string]: any } = parameters.reduce(
      (acc, param) => {
