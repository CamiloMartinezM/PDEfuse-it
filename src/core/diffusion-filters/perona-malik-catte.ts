        ...
        for (let i = 1; i <= nx; i++) {
            for (let j = 1; j <= ny; j++) {
                const newVal = f.getPixel(i, j)[0]
                    + rxx * ((g.getPixel(i + 1, j)[0] + g.getPixel(i, j)[0]) * (f.getPixel(i + 1, j)[0] - f.getPixel(i, j)[0])
                        + (g.getPixel(i - 1, j)[0] + g.getPixel(i, j)[0]) * (f.getPixel(i - 1, j)[0] - f.getPixel(i, j)[0]))
                    + ryy * ((g.getPixel(i, j + 1)[0] + g.getPixel(i, j)[0]) * (f.getPixel(i, j + 1)[0] - f.getPixel(i, j)[0])
                        + (g.getPixel(i, j - 1)[0] + g.getPixel(i, j)[0]) * (f.getPixel(i, j - 1)[0] - f.getPixel(i, j)[0]));
